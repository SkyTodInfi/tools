...
if __name__ == '__main__':
    ...
    for count,py,word in GTable:
        ...
        if word in dongxi_dict:
            dongxi_dict[word] = max(dongxi_dict[word], count)
        else:
            dongxi_dict[word] = count
        #f.write( unicode(word + ' %s' % count).encode('utf8') )#最终保存文件的编码，可以自给改
        #f.write('\n')
    for word, count in dongxi_dict.iteritems():
        f.write( unicode(word + ' %s' % count).encode('utf8') )#最终保存文件的编码，可以自给改
        f.write('\n')
    f.close()
